...
There, we provide a custom `ServiceInstanceListSupplier` with 3 hard-coded instances that Sprign Cloud LoadBalancer will choose from while making the calls to the "Say Hello" service.
...
If you have any `DiscoveryClient` on your classpath, the default Spring Cloud LoadBalancer configuration will use it under the hood to check for service instances.
...
You can see Ribbon's pings arriving every 15 seconds:
