...
an application and does a lot of the setup for you. For this guide, you need to run the
Spring Initializr twice, once for the `Say Hello` project and once for the `User` project.

The `Say Hello` project needs only the Spring Web dependency.
You can get a Maven build file with the necessary dependencies for the `Say Hello` project
directly from the https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.4.2.RELEASE&packaging=jar&jvmVersion=11&groupId=com.example&artifactId=spring-cloud-loadbalancer-say-hello&name=spring-cloud-loadbalancer-say-hello&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.spring-cloud-loadbalancer-say-hello&dependencies=web[Spring Initializr].
...
You can get a Gradle build file with the necessary dependencies for the `Say Hello` project
...
The `User` project needs the Cloud Loadbalancer and Spring Reactive Web dependencies.
You can get a Maven build file with the necessary dependencies for the `User` project
directly from the https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.4.2.RELEASE&packaging=jar&jvmVersion=11&groupId=com.example&artifactId=spring-cloud-loadbalancer-user&name=spring-cloud-loadbalancer-user&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.spring-cloud-loadbalancer-user&dependencies=cloud-loadbalancer,webflux[Spring Initializr].
The following listing shows the `pom.xml` file that is created when you choose Maven:
...
You can get a Gradle build file with the necessary dependencies for the `User` project
directly from the https://start.spring.io/#!type=gradle-project&language=java&platformVersion=2.4.2.RELEASE&packaging=jar&jvmVersion=11&groupId=com.example&artifactId=spring-cloud-loadbalancer-user&name=spring-cloud-loadbalancer-user&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.spring-cloud-loadbalancer-user&dependencies=cloud-loadbalancer,webflux[Spring Initializr].
The following listing shows the `build.gradle` file that is created when you choose Gradle:
