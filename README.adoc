...
For Gradle, you need a `settings.gradle` that includes the same directories:
...
If you use Maven for the `Say Hello` project, visit the https://start.spring.io/#!type=maven-project&language=java&platformVersion={spring_boot_version}&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=spring-cloud-loadbalancer-say-hello&name=spring-cloud-loadbalancer-say-hello&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.spring-cloud-loadbalancer-say-hello&dependencies=web[Spring Initializr] to generate a new project with the required dependency (Spring Web).
...
If you use Gradle for the `Say Hello` project, visit the https://start.spring.io/#!type=gradle-project&language=java&platformVersion={spring_boot_version}&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=spring-cloud-loadbalancer-say-hello&name=spring-cloud-loadbalancer-say-hello&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.spring-cloud-loadbalancer-say-hello&dependencies=web[Spring Initializr] to generate a new project with the required dependency (Spring Web).
...
If you use Maven for the `User` project, visit the https://start.spring.io/#!type=maven-project&language=java&platformVersion={spring_boot_version}&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=spring-cloud-loadbalancer-user&name=spring-cloud-loadbalancer-user&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.spring-cloud-loadbalancer-user&dependencies=cloud-loadbalancer,webflux[Spring Initializr] to generate a new project with the required dependencies (Cloud LoadBalancer and Spring Reactive Web).
...
If you use Gradle for the `User` project, visit the https://start.spring.io/#!type=gradle-project&language=java&platformVersion={spring_boot_version}&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=spring-cloud-loadbalancer-user&name=spring-cloud-loadbalancer-user&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.spring-cloud-loadbalancer-user&dependencies=cloud-loadbalancer,webflux[Spring Initializr] to generate a new project with the required dependencies (Cloud LoadBalancer and Spring Reactive Web).
...
It is a simple `@RestController`, where we have one `@GetMapping` method for the `/greeting` and another for the root path `/`.
...
In `UserApplication.java`, we have also added a `/hello` endpoint that does the same action. However, rather than use the `@LoadBalanced` annotation, we use an injected load-balancer exchange filter function (`lbFunction`), which we pass by using the `filter()` method to a `WebClient` instance that we programmatically build.

TIP: Even though we set up the load-balanced `WebClient` instances slightly differently for the two endpoints, the end behavior for both is exactly the same.
...
You can run other instances on ports 9092 and 9999.
To do so with Gradle, run the following command:
...
Then you can start the `User` service.
To do so, access `localhost:8888/hi` and watch the `Say Hello` service instances.
